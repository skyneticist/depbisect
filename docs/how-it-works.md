...
3. **Read lockfiles.** `package-lock.json` (v1–v3), `pnpm-lock.yaml` (v5/v6/v9), `yarn.lock` (classic v1 and Berry), `Cargo.lock`, `go.sum`, `uv.lock`, or `composer.lock` supplies exact resolved versions for display, and exposes *lockfile-only* changes — dependencies whose spec is unchanged but whose resolution moved. These cannot be bisected (see below) and are reported as diagnostics. pip has no separate lockfile; the exact `==` pins in `requirements.txt` play this role, so lockfile-only drift cannot occur there.
