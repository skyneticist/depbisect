...
6. **Delta-debug (ddmin).** Before every uncached trial, the owned worktree is reset to `--to` and all untracked and ignored files are removed. Candidate subsets are then applied by rewriting the manifest (structured edit, never regex), installing with npm/pnpm/yarn/cargo/go/uv, and re-running the command.
