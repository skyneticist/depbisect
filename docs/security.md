...
- Confines installation and verification writes to temporary directories it
  created (`$TMPDIR/depbisect-*`); the user worktree is never written to.
- Writes reports and the resumable checkpoint only to their documented
  user-selected paths.
...
- It never runs destructive git commands over your checkout. `git reset
  --hard` and `git clean -ffdx` are restricted to the DepBisect-owned
  temporary worktree before each trial.
